...
const DEFAULT_INNER_GRADIENT = 'linear-gradient(145deg, rgba(15, 23, 42, 0.3) 0%, rgba(0, 0, 0, 0.85) 100%)';
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  avatarUrl = '/profile_avatar.webp',
  iconUrl = '',
  grainUrl = '',
  innerGradient,
  behindGlowEnabled = true,
  behindGlowColor = 'rgba(255, 85, 0, 0.45)',
  behindGlowSize = '50%',
  className = '',
  enableTilt = true,
  enableMobileTilt = false,
  mobileTiltSensitivity = 5,
  miniAvatarUrl = '/profile_avatar.webp',
  name = 'Pavan Borigi',
  ...
  const updateDimensions = useCallback(() => {
    ...
    if (shell) {
      widthRef.current = shell.clientWidth || 1;
      heightRef.current = shell.clientHeight || 1;
    }
  }, []);
  ...
  const tiltEngine = useMemo<TiltEngine | null>(() => {
    ...
    return {
      ...
      toCenter(): void {
        const shell = shellRef.current;
        if (!shell) return;
        this.setTarget(widthRef.current / 2, heightRef.current / 2);
      },
      beginInitial(durationMs: number): void {
  ...
  useEffect(() => {
    ...
    const initialX = widthRef.current - ANIMATION_CONFIG.INITIAL_X_OFFSET;
  ...
  const cardStyle = useMemo(
    () => ({
      ...
      '--sunpillar-1': 'rgba(255, 255, 255, 0.3)',
      '--sunpillar-2': 'rgba(255, 85, 0, 0.15)',
      '--sunpillar-3': 'rgba(0, 255, 179, 0.12)',
      '--sunpillar-4': 'rgba(255, 255, 255, 0.3)',
      '--sunpillar-5': 'rgba(255, 85, 0, 0.15)',
      '--sunpillar-6': 'rgba(0, 255, 179, 0.12)',
